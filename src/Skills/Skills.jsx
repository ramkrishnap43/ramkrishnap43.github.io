import {
  Avatar,
  Box,
  Center,
  chakra,
  Img,
  SimpleGrid,
  Text,
} from "@chakra-ui/react";
import React from "react";

const Skills = () => {
  return (
    <Box id="skills" maxW="7xl" mx={"auto"} pt={5} px={{ base: 2, sm: 8, md: 12 }}>
      <chakra.h1
        textAlign={"center"}
        fontSize={"4xl"}
        py={10}
        fontWeight={"bold"}
      >
        Skills
      </chakra.h1>

      <SimpleGrid
        columns={{ base: 1, md: 3 }}
        spacing={{ base: 5, lg: 8 }}
        w="60%"
        m={"auto"}
      >
        <Box shadow={'xl'}>
          <Center>
            <Img
              w={"60%"}
              p="10%"
              src="https://portfolio-saravanakumarjn.vercel.app/static/media/html_5.e7638e36.svg"
              alt="HTML"
            />
          </Center>
          <Text
            fontSize={{ base: "xl", sm: "2xl", md: "2xl" }}
            fontWeight="bold"
          >
            HTML
          </Text>
        </Box>

        <Box shadow={'xl'}>
          <Center>
            <Img
              w={"60%"}
              p="10%"
              src="https://portfolio-saravanakumarjn.vercel.app/static/media/css_3.e7a9de36.svg"
              alt="CSS"
            />
          </Center>
          <Text
            fontSize={{ base: "xl", sm: "2xl", md: "2xl" }}
            fontWeight="bold"
          >
            CSS
          </Text>
        </Box>

        <Box shadow={'xl'}>
          <Center>
            <Img
              w={"60%"}
              p="10%"
              src="https://portfolio-saravanakumarjn.vercel.app/static/media/javascript.fd46ca41.svg"
              alt="Javascript"
            />
          </Center>
          <Text
            fontSize={{ base: "xl", sm: "2xl", md: "2xl" }}
            fontWeight="bold"
          >
            Javascript
          </Text>
        </Box>

        <Box shadow={'xl'}>
          <Center>
            <Img
              w={"60%"}
              p="10%"
              src="https://portfolio-saravanakumarjn.vercel.app/static/media/react.e27571ea.svg"
              alt="React"
            />
          </Center>
          <Text
            fontSize={{ base: "xl", sm: "2xl", md: "2xl" }}
            fontWeight="bold"
          >
            React
          </Text>
        </Box>

        <Box shadow={'xl'}>
          <Center>
            <Img
              w={"60%"}
              p="10%"
              src="https://portfolio-saravanakumarjn.vercel.app/static/media/redux.a9567540.svg"
              alt="Redux"
            />
          </Center>
          <Text
            fontSize={{ base: "xl", sm: "2xl", md: "2xl" }}
            fontWeight="bold"
          >
            Redux
          </Text>
        </Box>

        <Box shadow={'xl'}>
          <Center>
            <Img
              w={"60%"}
              p="10%"
              src="https://portfolio-saravanakumarjn.vercel.app/static/media/nodejs-icon.3dc364b8.svg"
              alt="Node-JS"
            />
          </Center>
          <Text
            fontSize={{ base: "xl", sm: "2xl", md: "2xl" }}
            fontWeight="bold"
          >
            Node-JS
          </Text>
        </Box>

        <Box shadow={'xl'}>
          <Center>
            <Img
              w={"60%"}
              p="10%"
              src="https://portfolio-saravanakumarjn.vercel.app/static/media/github-icon.04ed74f3.svg"
              alt="Github"
            />
          </Center>
          <Text
            fontSize={{ base: "xl", sm: "2xl", md: "2xl" }}
            fontWeight="bold"
          >
            Github
          </Text>
        </Box>

        <Box shadow={'xl'}>
          <Center>
            <Img
              w={"60%"}
              p="10%"
              src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTUdylODRwIQQjS7WKrCXU39C978mVDIni6Bg&usqp=CAU"
              alt="Bootstrap"
            />
          </Center>
          <Text
            fontSize={{ base: "xl", sm: "2xl", md: "2xl" }}
            fontWeight="bold"
          >
            Bootstrap
          </Text>
        </Box>

        <Box shadow={'xl'}>
          <Center>
            <Img
              w={"60%"}
              p="10%"
              src="data:image/png;base64,"
              alt="DSA"
            />
          </Center>
          <Text
            fontSize={{ base: "xl", sm: "2xl", md: "2xl" }}
            fontWeight="bold"
          >
            DSA
          </Text>
        </Box>
      </SimpleGrid>
    </Box>
  );
};

export default Skills;
